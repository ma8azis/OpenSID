	...
	(function() {
		...
		window.onload = function() {
			...
			var baseLayers = {
				...
				'Mapbox Streets': L.tileLayer('https://api.mapbox.com/v4/mapbox.streets/{z}/{x}/{y}@2x.png?access_token=<?= $this->setting->google_key ?>', {
				...
				'Mapbox Outdoors': L.tileLayer('https://api.mapbox.com/v4/mapbox.outdoors/{z}/{x}/{y}@2x.png?access_token=<?= $this->setting->google_key ?>', {
					attribution: '<a href="https://www.mapbox.com/about/maps">© Mapbox</a> <a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="https://mapbox.com/map-feedback/">Improve this map</a>'
				}),
				'Mapbox Streets Satellite': L.tileLayer('https://api.mapbox.com/v4/mapbox.streets-satellite/{z}/{x}/{y}@2x.png?access_token=<?= $this->setting->google_key ?>', {
					attribution: '<a href="https://www.mapbox.com/about/maps">© Mapbox</a> <a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="https://mapbox.com/map-feedback/">Improve this map</a>'
				}),
			};
			...
			<?php if ($layer_desa == 1 and !empty($desa['path'])) : ?>
				...
				semua_marker.push(turf.polygon(daerah_desa, {
					content: content_desa,
					style: style_polygon
				}))
				//Menambahkan point kantor desa
				semua_marker.push(turf.point([<?= $desa['lng'] . "," . $desa['lat'] ?>], {
					content: "Kantor Desa"
				}))
			<?php endif; ?>
			...
			<?php if ($layer_dusun == 1 and !empty($dusun_gis)) : ?>
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_dusun[x].path) {
						...
						//Menambahkan poligon ke marker
						semua_marker.push(turf.polygon(daftar_dusun[x].path, {
							content: content_dusun,
							style: dusun_style
						}));
						//Menambahkan point kantor desa
						if (daftar_dusun[x].lng) {
							semua_marker.push(turf.point([daftar_dusun[x].lng, daftar_dusun[x].lat], {
								content: "Kantor Dusun " + daftar_dusun[x].dusun
							}))
						}
					}
				}
			...
			<?php if ($layer_rw == 1 and !empty($rw_gis)) : ?>
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_rw[x].path) {
						...
						//Menambahkan poligon ke marker
						semua_marker.push(turf.polygon(daftar_rw[x].path, {
							content: content_rw,
							style: rw_style
						}));
						//Menambahkan point kantor desa
						if (daftar_rw[x].lng) {
							semua_marker.push(turf.point([daftar_rw[x].lng, daftar_rw[x].lat], {
								content: "Kantor RW " + daftar_rw[x].rw
							}))
						}
					}
				}
			...
			<?php if ($layer_rt == 1 and !empty($rt_gis)) : ?>
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_rt[x].path) {
						...
						//Menambahkan poligon ke marker
						semua_marker.push(turf.polygon(daftar_rt[x].path, {
							content: content_rt,
							style: rt_style
						}));
						//Menambahkan point kantor desa
						if (daftar_rt[x].lng) {
							semua_marker.push(turf.point([daftar_rt[x].lng, daftar_rt[x].lat], {
								content: "Kantor RT " + daftar_rt[x].rt
							}))
						}
					}
				}
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_lokasi[x].lat) {
						...
						content = '<div id="content">' +
							...
							'</div>' +
							'</div>';
						semua_marker.push(turf.point([daftar_lokasi[x].lng, daftar_lokasi[x].lat], {
							content: content,
			...
			<?php if ($layer_area == 1 and !empty($area)) : ?>
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_area[x].path) {
						...
						semua_marker.push(turf.polygon(daftar_area[x].path, {
			...
			<?php if ($layer_garis == 1 and !empty($garis)) : ?>
				...
				for (var x = 0; x < jml; x++) {
					if (daftar_garis[x].path) {
						...
						semua_marker.push(turf.lineString(coords, {
