<script>
	...
	window.onload = function() {
		...
		<?php if (!empty($janda['lat'] && !empty($janda['lng']))) : ?>
			var posisi = [<?= $janda['lat'] . "," . $janda['lng'] ?>];
			var zoom = <?= $janda['zoom'] ?: 16 ?>;
		<?php else : ?>
		...
		var kantor_dusun = L.marker(posisi, {
			draggable: true
		}).addTo(peta_dusun);
...
<div class="content-wrapper">
	<section class="content-header">
		<h1>Lokasi Janda <?= ucwords($this->setting->sebutan_desa . " " . $desa['nama_desa']) ?></h1>
		<ol class="breadcrumb">
			<li><a href="<?= site_url('hom_sid') ?>"><i class="fa fa-home"></i> Home</a></li>
			<li><a href="<?= site_url('sid_core') ?>"> Daftar <?= ucwords($this->setting->sebutan_dusun) ?></a></li>
			<li class="active">Lokasi Janda</li>
		</ol>
	</section>
					...
					<form id="validasi" action="<?= $form_action ?>" method="POST" enctype="multipart/form-data" class="form-horizontal">
						<div class="box-body">
							<div class="row">
								<div class="col-sm-12">
									<div id="mapx">
										<input type="hidden" name="zoom" id="zoom" value="<?= $janda['zoom'] ?>" />
										<input type="hidden" name="map_tipe" id="map_tipe" value="<?= $janda['map_tipe'] ?>" />
										<input type="hidden" name="id" id="id" value="<?= $janda['id'] ?>" />
									</div>
								</div>
							</div>
						</div>
						<div class='box-footer'>
							<div class='col-xs-12'>
								<div class="form-group">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control number" name="lat" id="lat" value="<?= $janda['lat'] ?>" />
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label" for="lat">Lng</label>
									<div class="col-sm-9">
										<input type="text" class="form-control number" name="lng" id="lng" value="<?= $janda['lng'] ?>" />
									</div>
								</div>
