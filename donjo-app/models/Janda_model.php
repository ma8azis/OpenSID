<?php class Janda_model extends CI_Model
{
	...
	*/
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('janda', $data);
	...
	{
		$data = $this->db->where('rt', '0')->where('rw', '0')->get('tweb_wil_clusterdesa')->result_array();
		return $data;
	}

	public function list_rw($dusun = '')
	{
		$data = $this->db->where('rt', '0')->where('dusun', urldecode($dusun))->where('rw <>', '0')->order_by('rw')->get('tweb_wil_clusterdesa')->result_array();
		return $data;
	}
	...
	{
		$data = $this->db->where('rt <>', '0')->where('dusun', urldecode($dusun))->where('rw', $rw)->order_by('rt')->get('tweb_wil_clusterdesa')->result_array();
		return $data;
	}
